...
init_sequential_hybrid <- function(fdata,
  ...
  errors <- 10^60
  iter <- 0
  while (errors > tol && iter < max_iter) {
    iter <- iter+1
    u_old <- 0
    if (!is.null(fdata)) {
...
init_joint_hybrid = function(fdata, nfdata, S_smooth = NULL, S_2_inverse = NULL, G_half_inverse = NULL, G_half = NULL, n = n, tol,max_iter){
...
cv_local_hybrid <- function(
  ...
  if (pen_u) {
    ...
    for (k in seq_len(nfold_u)) {
      ...
      fold_errors_u[k] <- fv_error + nfv_error
    }
    ...
    errs_u <- fold_errors_u/normalization_factor
    mean_u <- mean(errs_u)
    sd_u <- sd(errs_u)
    se_u <- sd_u/sqrt(nfold_u)
  } else {
    mean_u <- sd_u <- se_u <- NULL
  }
  ...
  if (pen_nfd) {
    ...
    errs_nfd <- fold_errors_nfd/normalization_factor
    mean_nfd <- mean(errs_nfd)
    sd_nfd <- sd(errs_nfd)
    se_nfd <- sd_nfd/sqrt(nfold_nfd)

  } else {
    mean_nfd <- sd_nfd <- se_nfd <- NULL
  }
  ...
  if (pen_fd) {
    ...
    errs_fd <- fold_errors_fd/normalization_factor
    mean_fd <- mean(errs_fd)
    sd_fd <- sd(errs_fd)
    se_fd <- sd_fd/sqrt(nfold_fd)

  } else {
    mean_fd <- sd_fd <- se_fd <- NULL
  }

  #--------------------------------
  # Return the same output structure
  #--------------------------------
  list(err_u = err_u, err_nfd = err_nfd,err_fd = err_fd, se_u = se_u, se_nfd = se_nfd, se_fd = se_fd)
}
...
handle_sparse_tuning_hybrid <- function(
  ...
  if (!is.null(sparse_tuning_u) && isTRUE(pen_u)) {
    ...
    cv_scores_u_mat <- do.call(rbind, cv_scores_u_list)
    cv_scores_u <- cv_scores_u_mat[ , "err_u" ]
    cv_se_u     <- cv_scores_u_mat[ , "se_u"  ]

    #cv_scores_u    <- unlist(cv_scores_u_list)
    #best_index_u   <- which.min(cv_scores_u)
    if (cv.pick == "1se"){
      min_idx <- which.min(cv_scores_u)
      cutoff <- cv_scores_u[min_idx] + cv_se_u[min_idx]
      idx_within <- which(cv_scores_u <= cutoff)
      best_index_u <- max(idx_within)
    } else {
      best_index_u   <- which.min(cv_scores_u)
    }
...
gcv_joint_hybrid <- function(fdata, nfdata, G_half, G, S_smooth, S_2_inverse, G_half_inverse, hd_obj, smooth_tuning, n,tol,max_iter) {
  mvmfd_obj <- hd_obj$mf
  CV_score_smooth <- Inf
  pb <- txtProgressBar(min = 0, max = dim(smooth_tuning)[1], style = 3, width = 50, char = "=")
  smooth_tuning_result <- handle_smooth_tuning_hybrid(fdata, nfdata, G_half, G, S_smooth, S_2_inverse, G_half_inverse, hd_obj, smooth_tuning = smooth_tuning, CV_score_smooth = CV_score_smooth, power_type = "joint", n = n, pb = pb, count = 0,tol = tol, max_iter = max_iter)
    ...
    for (i in 1:n) {
      ...
      if (!is.null(sparse_tuning_nfd)) {
        if (sparse_CV == FALSE) {
          ...
          sparse_tuning_temp_nfd <- do.call(cbind,sparse_tuning_nfd)[i,]
        } else {
          #sparse_tuning_temp_nfd <- as.matrix(expand.grid(sparse_tuning_nfd))
          sparse_tuning_temp_nfd <- do.call(cbind,sparse_tuning_nfd)
        }
    ...
    for (i in 1:n) {
      ...
      if (!is.null(sparse_tuning_fd)) {
        if (sparse_CV == FALSE) {
          # Convert the list/vector into a matrix and extract the i-th row as a 1-row matrix.
          #sparse_tuning_temp_fd <- as.matrix(expand.grid(lapply(sparse_tuning_fd,function(x) x[i])))
          sparse_tuning_temp_fd <- do.call(cbind,sparse_tuning_fd)[i,]
          
        } else {
          #sparse_tuning_temp_fd <- as.matrix(expand.grid(sparse_tuning_fd))
          sparse_tuning_temp_fd <- do.call(cbind,sparse_tuning_fd)
...
joint_power_hybrid <- function(hd_obj, n, smooth_tuning, smooth_tuning_type, centerfns, alpha_orth,tol,max_iter) {
  ...
  cv_result = gcv_joint_hybrid(
    ...
    G_half_inverse = G_half_inverse, 
    S_smooth = S_smooth, 
    S_2_inverse = S_2_inverse, n = n,tol = tol, max_iter = max_iter
  )
